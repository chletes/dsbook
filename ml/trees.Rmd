...
```{r curse-of-dim, out.width="50%", fig.height=1.5}
rafalib::mypar()
x <- seq(0,1,len=100)
y <- rep(1, 100)
...
```{r curse-of-dim-2, fig.width=7, fig.height=3.5, out.width="50%",}
rafalib::mypar(1,2)
...
```{r olive-tree, warning=FALSE, message=FALSE, fig.height=4.5, out.width="50%"}
library(caret)
library(rpart)
rafalib::mypar()
...
```{r polls-2008-tree, fig.height=5, out.width="60%"}
rafalib::mypar()
plot(fit, margin = 0.1)
text(fit, cex = 0.75)
```
...
```{r polls-2008-final-model, fig.height=5, out.width="80%"}
rafalib::mypar()
plot(train_rpart$finalModel, margin = 0.1)
text(train_rpart$finalModel, cex = 0.75)
```
...
```{r rf-cond-prob, out.width="100%", warning=FALSE, message=FALSE}
library(gridExtra)
...
```{r cond-prob-final-rf, out.width="100%"}
p1 <- plot_cond_prob() + ggtitle("True conditional probability")
