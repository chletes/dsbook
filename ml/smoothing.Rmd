...
```{r signal-plus-noise-example, message=FALSE, warning=FALSE, fig.height=6}
library(tidyverse)
set.seed(1)
n <- 100
...
```{r binsmoother-animation, warning=FALSE} 
# library(gganimate)
span <- 7
...
#   knitr::include_graphics(file.path(img_path,"binsmooth-anim.gif"))
# } else{
...
```{r gaussian-kernel, out.width="80%", fig.height=3, fig.width=6}
x_0 <- -125
...
```{r kernel-animation, warning=FALSE} 
tmp <- polls_2008 %>%
  crossing(center = polls_2008$day) %>%
  mutate(dist = abs(day - center)) %>%
  filter(dist <= span) %>% 
...
```{r, echo=FALSE}
 knitr::include_graphics("ml/img/garden.png")
```
...
```{r loess-animation, warning=FALSE} 
library(broom)
...
```{r loess-multi-span-animation, eval=FALSE}
...
```{r triweight-kernel, out.width="80%", fig.height=3, fig.width=6}
x_0 <- -125
