...
<!--
```{r simulated-twin-heights, fig.width=3, fig.height=3, , message=FALSE, out.width="50%"}
lim <- c(48, 78)
rafalib::mypar()
...
```{r distance-illustration, fig.width=3, fig.height=3, , out.width="40%"}
rafalib::mypar()
plot(x, xlim=lim, ylim=lim)
...
```{r rotation, fig.width=3, fig.height=3, out.width="40%"}
rafalib::mypar()
...
```{r distance-approx-2, fig.width=3, fig.height=3, out.width="40%"}
rafalib::mypar()
...
```
<!--
```{r orthogonal-transformation, fig.width=3, fig.height=3}
rafalib::mypar()
plot(dist(x), dist(z))
abline(0, 1, col = "red")
```
...
```{r illustrate-pca-twin-heights, height = 5, out.width="70%"}
...
```{r iris-distances, fig.width = 4, fig.height = 4, out.width="50%"}
rafalib::mypar()
x <- iris[,1:4] %>% as.matrix()
d <- dist(x)
image(as.matrix(d), col = rev(RColorBrewer::brewer.pal(9, "RdBu")))
...
```{r illustrate-pca-twin-heights-iris, fig.height = 6, out.width="70%"}
rafalib::mypar()
illustrate_pca(x)
```
