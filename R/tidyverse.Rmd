...
In the call to `select`, the first argument `murders` is an object, but `state`, `region`, and `rate` are variable names from that table. 
...
You can add columns using the __dplyr__ function `mutate`.
...
murders %>% 
  ...
  filter(region %in% c("Notheast", "West") & rate < 1) %>%
  select(state, rate, rank)
```
